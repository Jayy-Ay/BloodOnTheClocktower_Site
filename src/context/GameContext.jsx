...
const initialState = {
  ...
  scriptName: '',
  scriptIcon: '',
  savedScripts: [], // { id, name, icon, characters, dateAdded }
  players: [],
  characterBag: [],
  drawnCharacters: [],
  ...
  switch (action.type) {
    case 'LOAD_SCRIPT': {
      return {
        ...
        scriptIcon: action.payload.icon || '',
        characterBag: [],
        drawnCharacters: [],
      }
    }
    case 'SAVE_CUSTOM_SCRIPT': {
      const newScript = {
        id: action.payload.id || Date.now().toString(),
        name: action.payload.name,
        icon: action.payload.icon || '📜',
        characters: action.payload.characters,
        dateAdded: action.payload.dateAdded || Date.now(),
      }
      // Remove any existing script with the same id
      const filtered = state.savedScripts.filter(s => s.id !== newScript.id)
      return {
        ...state,
        savedScripts: [...filtered, newScript],
      }
    }
    case 'DELETE_CUSTOM_SCRIPT': {
      return {
        ...state,
        savedScripts: state.savedScripts.filter(s => s.id !== action.payload),
      }
    }
    case 'UPDATE_CUSTOM_SCRIPT': {
      return {
        ...state,
        savedScripts: state.savedScripts.map(s =>
